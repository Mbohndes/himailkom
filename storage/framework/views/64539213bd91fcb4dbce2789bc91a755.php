...
<?php $__env->startSection('title', 'Arsip Digital HIMA'); ?>
...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <?php if($errors->any()): ?>
        <div class="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-xl text-sm font-medium shadow-sm">
            <?php echo e($errors->first() ?? 'Gagal mengarsipkan data. Pastikan format input benar.'); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100 lg:sticky lg:top-6">
            ...
            <form action="<?php echo e(route('superadmin.archives.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-4">
                <?php echo csrf_field(); ?>
                <div>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <form action="<?php echo e(route('superadmin.archives.index')); ?>" method="GET" class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-3">
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama dokumen..." class="flex-1 bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-2.5">
                ...
                <?php if(request()->anyFilled(['search', 'category'])): ?>
                    <a href="<?php echo e(route('superadmin.archives.index')); ?>" class="px-4 py-2.5 bg-red-50 text-red-600 rounded-xl text-sm font-bold flex items-center justify-center">X</a>
                <?php endif; ?>
            </form>
            ...
            <div class="bg-white rounded-[30px] shadow-sm border border-slate-100 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm text-slate-600 divide-y divide-slate-100">
                            <?php $__empty_1 = true; $__currentLoopData = $archives; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $arsip): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-800 mb-1"><?php echo e($arsip->title); ?></div>
                                    <?php if($arsip->description): ?>
                                        <div class="text-[11px] text-slate-400 truncate w-48"><?php echo e($arsip->description); ?></div>
                                    <?php endif; ?>
                                </td>
                                
                                <td class="px-6 py-4 font-semibold text-xs">
                                    <?php if($arsip->drive_link): ?>
                                ...
                                <td class="px-6 py-4">
                                    <span class="bg-indigo-50 text-[#5442F5] text-xs font-extrabold px-2.5 py-1 rounded-md"><?php echo e($arsip->category); ?></span>
                                end</td>
                                
                                <td class="px-6 py-4 flex justify-end gap-2">
                                    <?php if($arsip->drive_link): ?>
                                        <a href="<?php echo e($arsip->drive_link); ?>" target="_blank" class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Buka Google Drive">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                        </a>
                                    <?php endif; ?>
                                    <?php if($arsip->file_path): ?>
                                        <a href="<?php echo e(asset('storage/' . $arsip->file_path)); ?>" target="_blank" download class="p-2 text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors" title="Unduh File Lokal">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                        </a>
                                    <?php endif; ?>
                                    
                                    <form action="<?php echo e(route('superadmin.archives.destroy', $arsip->id)); ?>" method="POST" onsubmit="return confirm('Hapus dokumen ini dari rekam arsip?');">
                ...
                <div class="p-4 border-t border-slate-100 bg-slate-50/30"><?php echo e($archives->links()); ?></div>
            </div>
        </div>
    </div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/archives/index.blade.php ENDPATH**/ ?>