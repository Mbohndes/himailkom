...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
   ...
   <form method="GET" action="<?php echo e(route('superadmin.finance.incomes.index')); ?>" class="w-full">
    <div class="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div class="flex flex-wrap gap-3 items-center w-full sm:w-auto justify-end">
            ...
            <?php if(request('month') || request('year')): ?>
                <a href="<?php echo e(route('superadmin.finance.incomes.index')); ?>" class="text-xs font-bold text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap h-[40px] flex items-center">
                    Reset
                </a>
            <?php endif; ?>
        </div>
    ...
    <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm overflow-hidden">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-6 py-4 font-bold text-[#5442F5]"><?php echo e($inc->source); ?></td>
                    <td class="px-6 py-4 font-semibold text-slate-800"><?php echo e($inc->description); ?></td>
                    <td class="px-6 py-4 text-slate-600"><?php echo e(\Carbon\Carbon::parse($inc->date)->format('d M Y')); ?></td>
                    <td class="px-6 py-4 font-bold text-[#14C95A]">Rp <?php echo e(number_format($inc->amount, 0, ',', '.')); ?></td>
                    <td class="px-6 py-4 text-slate-500 text-sm"><?php echo e($inc->user->name ?? 'Admin'); ?></td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <button onclick="document.getElementById('modal-edit-<?php echo e($inc->id); ?>').classList.remove('hidden')" class="p-2 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded-xl transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </button>
                            <form action="<?php echo e(route('superadmin.finance.incomes.destroy', $inc->id)); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus data pemasukan ini?')">
                                <?php echo csrf_field(); ?>
                ...
                <div id="modal-edit-<?php echo e($inc->id); ?>" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div class="bg-white rounded-3xl p-8 w-full max-w-md text-left">
                        ...
                        <form action="<?php echo e(route('superadmin.finance.incomes.update', $inc->id)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select name="source" class="w-full rounded-xl border-slate-200 text-slate-700" required>
                                        <option value="Sponsorship" <?php echo e($inc->source == 'Sponsorship' ? 'selected' : ''); ?>>Sponsorship</option>
                                        <option value="Dana Fakultas" <?php echo e($inc->source == 'Dana Fakultas' ? 'selected' : ''); ?>>Dana Fakultas</option>
                                        <option value="Donasi Alumni" <?php echo e($inc->source == 'Donasi Alumni' ? 'selected' : ''); ?>>Donasi Alumni</option>
                                        <option value="Penjualan Merchandise" <?php echo e($inc->source == 'Penjualan Merchandise' ? 'selected' : ''); ?>>Penjualan Merchandise</option>
                                        <option value="Hasil Usaha / Danus" <?php echo e($inc->source == 'Hasil Usaha / Danus' ? 'selected' : ''); ?>>Hasil Usaha / Danus</option>
                                        <option value="Lainnya" <?php echo e($inc->source == 'Lainnya' ? 'selected' : ''); ?>>Lainnya</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="text-xs font-bold text-slate-400 block mb-1">Deskripsi</label>
                                    <input type="text" name="description" value="<?php echo e($inc->description); ?>" class="w-full rounded-xl border-slate-200" required>
                                </div>
                                <div>
                                    <label class="text-xs font-bold text-slate-400 block mb-1">Jumlah (Rp)</label>
                                    <input type="number" name="amount" value="<?php echo e(intval($inc->amount)); ?>" class="w-full rounded-xl border-slate-200" required>
                                </div>
                                <div>
                                    <label class="text-xs font-bold text-slate-400 block mb-1">Tanggal Transaksi</label>
                                    <input type="date" name="date" value="<?php echo e($inc->date); ?>" class="w-full rounded-xl border-slate-200" required>
                                </div>
                                <div class="flex gap-3 mt-4">
                                    <button type="button" onclick="document.getElementById('modal-edit-<?php echo e($inc->id); ?>').classList.add('hidden')" class="flex-1 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold">Batal</button>
        ...
        <div class="p-4 border-t border-slate-100"><?php echo e($incomes->links()); ?></div>
    </div>
</div>
...
<div id="modal-tambah" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl p-8 w-full max-w-md">
        ...
        <form action="<?php echo e(route('superadmin.finance.incomes.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="space-y-4">
                <select name="source" class="w-full rounded-xl border-slate-200 text-slate-700" required>
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/incomes/index.blade.php ENDPATH**/ ?>