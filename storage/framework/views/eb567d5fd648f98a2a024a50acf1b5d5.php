...
<?php $__env->startSection('title', 'Laporan Keaktifan Anggota'); ?>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $reportData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="p-4 pl-6">
                                <?php if($index == 0): ?> <span class="text-2xl">🥇</span>
                                <?php elseif($index == 1): ?> <span class="text-2xl">🥈</span>
                                <?php elseif($index == 2): ?> <span class="text-2xl">🥉</span>
                                <?php else: ?> <span class="text-sm font-black text-slate-400 ml-2">#<?php echo e($index + 1); ?></span>
                                <?php endif; ?>
                            </td>
                            
                            <!-- Nama & Role -->
                            <td class="p-4">
                                <p class="text-sm font-black text-slate-800"><?php echo e($row->name); ?></p>
                                <p class="text-[11px] font-bold text-slate-400 mt-0.5"><?php echo e($row->role); ?></p>
                            </td>
                            
                            <!-- Total Hadir -->
    <td class="p-4 text-center">
        <span class="text-sm font-black text-slate-700"><?php echo e($row->total_hadir); ?></span>
        <span class="text-xs font-bold text-slate-400">/ <?php echo e($row->total_agenda); ?> Agenda</span>
    </td>
                            ...
                            <td class="p-4 w-64">
                                <div class="flex items-center gap-3">
                                    <div class="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full <?php echo e($row->persentase >= 50 ? 'bg-[#14C95A]' : 'bg-[#F59E0B]'); ?>" style="width: <?php echo e($row->persentase); ?>%;"></div>
                                    </div>
                                    <span class="text-xs font-black text-slate-700 w-8"><?php echo e($row->persentase); ?>%</span>
                                </div>
                            </td>

                            <!-- Label Keterangan -->
                            <td class="p-4 pr-6">
                                <span class="px-3 py-1 text-[11px] font-black uppercase tracking-wider rounded-lg border <?php echo e($row->warna); ?>">
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/reports/activity.blade.php ENDPATH**/ ?>