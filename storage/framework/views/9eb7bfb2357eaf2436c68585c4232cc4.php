...
<?php $__env->startSection('title', 'Galeri HIMA'); ?>
...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <?php if (! \Illuminate\Support\Facades\Blade::check('role', 'Anggota')): ?>
            <a href="<?php echo e(route('superadmin.gallery.create')); ?>" class="px-5 py-2.5 bg-[#5442F5] hover:bg-[#4331e5] hover:-translate-y-0.5 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <?php $__empty_1 = true; $__currentLoopData = $albums; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $album): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 group">
                ...
                <div class="w-full aspect-[4/3] bg-slate-200 relative overflow-hidden bg-cover bg-center" style="background-image: url('<?php echo e($album->thumbnail ? asset('storage/'.$album->thumbnail) : 'https://ui-avatars.com/api/?name='.urlencode($album->title).'&background=5442F5&color=fff&size=512'); ?>')">
                    ...
                    <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[2px]">
                        <a href="<?php echo e($album->drive_link); ?>" target="_blank" class="px-5 py-2.5 bg-white text-[#5442F5] rounded-xl font-bold text-sm shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 flex items-center gap-2">
                            Buka Drive
                ...
                <div class="p-5 relative">
                    <h3 class="font-extrabold text-slate-800 text-base leading-tight mb-2 line-clamp-2"><?php echo e($album->title); ?></h3>
                    ...
                    <div class="flex items-center justify-between mt-4 text-xs font-medium text-slate-400">
                        <span class="flex items-center gap-1">
                            ...
                            <?php echo e($album->activity_date ? \Carbon\Carbon::parse($album->activity_date)->format('d M Y') : '-'); ?>

                        </span>
                        
                        <?php if (! \Illuminate\Support\Facades\Blade::check('role', 'Anggota')): ?>
                            <form action="<?php echo e(route('superadmin.gallery.destroy', $album->id)); ?>" method="POST" onsubmit="return confirm('Hapus album ini?');">
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/gallery/index.blade.php ENDPATH**/ ?>