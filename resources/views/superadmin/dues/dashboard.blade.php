    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="bg-gradient-to-r from-[#5442F5] to-indigo-800 rounded-[32px] p-8 md:p-10 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="relative z-10">
                ...
                <h2 class="text-4xl md:text-5xl font-black tracking-tight">Rp {{ number_format($saldoAkhir, 0, ',', '.') }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100 flex items-center gap-5">
                ...
                <div>
                    ...
                    <h3 class="text-xl font-black text-slate-800">Rp {{ number_format($totalIuranMasuk, 0, ',', '.') }}</h3>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100 flex items-center gap-5">
                ...
                <div>
                    ...
                    <h3 class="text-xl font-black text-slate-800">Rp {{ number_format($totalPemasukan, 0, ',', '.') }}</h3>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100 flex items-center gap-5">
                ...
                <div>
                    ...
                    <h3 class="text-xl font-black text-slate-800">Rp {{ number_format($totalPengeluaran, 0, ',', '.') }}</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                <h3 class="text-3xl font-black text-slate-800">Rp {{ number_format($totalTarget, 0, ',', '.') }}</h3>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-green-50 p-6 rounded-[24px] border border-emerald-100 shadow-sm relative overflow-hidden">
                ...
                <h3 class="text-3xl font-black text-emerald-700">Rp {{ number_format($totalIuranMasuk, 0, ',', '.') }}</h3>
            </div>
            ...
            <div class="bg-gradient-to-br from-amber-50 to-orange-50 p-6 rounded-[24px] border border-amber-100 shadow-sm relative overflow-hidden">
                ...
                <h3 class="text-3xl font-black text-amber-700">{{ $totalTunggakan }} <span class="text-lg font-bold text-amber-600">Transaksi</span></h3>
            </div>
        </div>
            ...
            <div class="bg-white rounded-[30px] p-8 border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center h-full min-h-[350px]">
                ...
                <div class="relative w-44 h-44 flex items-center justify-center mb-6">
                    <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                        ...
                        <path class="text-[#2CE574] transition-all duration-1000 ease-out" stroke-dasharray="{{ $rasioKolektibilitas }}, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"></path>
                    </svg>
                    <div class="absolute text-5xl font-black text-slate-800 tracking-tighter">{{ $rasioKolektibilitas }}<span class="text-2xl">%</span></div>
                </div>
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex items-end justify-between border-t border-white/10 pt-6">
                        <div>
                            ...
                            <h4 class="text-3xl font-black">{{ $totalAnggota }} <span class="text-lg font-bold text-slate-300">Jiwa</span></h4>
                        </div>
                        <a href="{{ route('superadmin.finance.payments') }}" class="px-6 py-3 bg-white text-slate-800 hover:bg-slate-100 hover:scale-105 active:scale-95 rounded-xl text-sm font-extrabold shadow-lg transition-all flex items-center gap-2 whitespace-nowrap">
                            Kelola Tagihan &rarr;
                        </a>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm p-7">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('superadmin.finance.incomes.index') }}" class="text-xs font-bold text-[#5442F5] hover:underline">Lihat Semua</a>
                </div>
                <div class="space-y-4">
                    @forelse($recentIncomes as $inc)
                        <div class="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-slate-800">{{ $inc->source }}</h4>
                                    <p class="text-xs text-slate-400 font-medium">{{ \Carbon\Carbon::parse($inc->date)->format('d M Y') }}</p>
                                </div>
                            </div>
                            <span class="font-bold text-[#14C95A] text-sm">+ Rp {{ number_format($inc->amount, 0, ',', '.') }}</span>
                        </div>
                    @empty
            ...
            <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm p-7">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('superadmin.finance.expenses.index') }}" class="text-xs font-bold text-[#5442F5] hover:underline">Lihat Semua</a>
                </div>
                <div class="space-y-4">
                    @forelse($recentExpenses as $ex)
                        <div class="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-slate-800">{{ $ex->description }}</h4>
                                    <p class="text-xs text-slate-400 font-medium">{{ \Carbon\Carbon::parse($ex->date)->format('d M Y') }}</p>
                                </div>
                            </div>
                            <span class="font-bold text-[#FF4D4D] text-sm">- Rp {{ number_format($ex->amount, 0, ',', '.') }}</span>
                        </div>
                    @empty
