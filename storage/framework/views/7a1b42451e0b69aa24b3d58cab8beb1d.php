...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center justify-between">
        ...
        <form method="GET" action="<?php echo e(route('superadmin.finance.expenses.index')); ?>" class="flex gap-3 items-center">
            ...
            <?php if(request('month') || request('year')): ?>
                <a href="<?php echo e(route('superadmin.finance.expenses.index')); ?>" class="text-xs font-bold text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 px-3 py-2.5 rounded-xl transition-colors">
                    Reset
                </a>
            <?php endif; ?>
        </form>
    ...
    <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm overflow-hidden">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
                <?php $__empty_1 = true; $__currentLoopData = $expenses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ex): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-slate-50">
                    <td class="px-6 py-4 font-bold text-slate-800"><?php echo e($ex->description); ?></td>
                    <td class="px-6 py-4 text-slate-600"><?php echo e(\Carbon\Carbon::parse($ex->date)->format('d M Y')); ?></td>
                    <td class="px-6 py-4 font-bold text-red-600">Rp <?php echo e(number_format($ex->amount, 0, ',', '.')); ?></td>
                    <td class="px-6 py-4 text-slate-500 text-sm"><?php echo e($ex->user->name ?? 'Admin'); ?></td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <button onclick="document.getElementById('modal-edit-<?php echo e($ex->id); ?>').classList.remove('hidden')" class="p-2 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded-xl transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </button>
                            <form action="<?php echo e(route('superadmin.finance.expenses.destroy', $ex->id)); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus data pengeluaran ini?')">
                                <?php echo csrf_field(); ?>
                ...
                <div id="modal-edit-<?php echo e($ex->id); ?>" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div class="bg-white rounded-3xl p-8 w-full max-w-md text-left">
                        ...
                        <form action="<?php echo e(route('superadmin.finance.expenses.update', $ex->id)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PUT'); ?>
                            <div class="space-y-4">
                                <div>
                                    <label class="text-xs font-bold text-slate-400 block mb-1">Keperluan / Deskripsi</label>
                                    <input type="text" name="description" value="<?php echo e($ex->description); ?>" class="w-full rounded-xl border-slate-200" required>
                                </div>
                                <div>
                                    <label class="text-xs font-bold text-slate-400 block mb-1">Jumlah (Rp)</label>
                                    <input type="number" name="amount" value="<?php echo e(intval($ex->amount)); ?>" class="w-full rounded-xl border-slate-200" required>
                                </div>
                                <div>
                                    <label class="text-xs font-bold text-slate-400 block mb-1">Tanggal Transaksi</label>
                                    <input type="date" name="date" value="<?php echo e($ex->date); ?>" class="w-full rounded-xl border-slate-200" required>
                                </div>
                                <div class="flex gap-3 mt-4">
                                    <button type="button" onclick="document.getElementById('modal-edit-<?php echo e($ex->id); ?>').classList.add('hidden')" class="flex-1 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold">Batal</button>
        ...
        <div class="p-4 border-t border-slate-100"><?php echo e($expenses->links()); ?></div>
    </div>
</div>
...
<div id="modal-tambah" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl p-8 w-full max-w-md">
        ...
        <form action="<?php echo e(route('superadmin.finance.expenses.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="space-y-4">
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/expenses/index.blade.php ENDPATH**/ ?>