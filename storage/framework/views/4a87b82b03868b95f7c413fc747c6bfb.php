...
<?php $__env->startSection('title', 'Dashboard Berita & Publikasi'); ?>
...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('superadmin.news.index')); ?>" class="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-slate-700 rounded-xl font-bold text-sm shadow-sm transition-all">
                Semua Berita
            </a>
            <a href="<?php echo e(route('superadmin.news.create')); ?>" class="px-5 py-2.5 bg-[#5442F5] hover:bg-[#4331e5] hover:-translate-y-0.5 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-gradient-to-br from-white to-slate-50 rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col justify-center hover:-translate-y-1 hover:shadow-md transition-all">
            ...
            <h2 class="text-3xl font-extrabold text-slate-800"><?php echo e($stats['total']); ?></h2>
        </div>
        <div class="bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl p-5 shadow-sm border border-emerald-100 flex flex-col justify-center hover:-translate-y-1 hover:shadow-md transition-all">
            ...
            <h2 class="text-3xl font-extrabold text-emerald-700"><?php echo e($stats['published']); ?></h2>
        </div>
        <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-5 shadow-sm border border-amber-100 flex flex-col justify-center hover:-translate-y-1 hover:shadow-md transition-all">
            ...
            <h2 class="text-3xl font-extrabold text-amber-700"><?php echo e($stats['draft']); ?></h2>
        </div>
        <div class="bg-gradient-to-br from-blue-50 to-sky-50 rounded-2xl p-5 shadow-sm border border-blue-100 flex flex-col justify-center hover:-translate-y-1 hover:shadow-md transition-all">
            ...
            <h2 class="text-3xl font-extrabold text-blue-700"><?php echo e($stats['scheduled']); ?></h2>
        </div>
        <div class="bg-gradient-to-br from-[#111111] to-slate-800 rounded-2xl p-5 shadow-sm flex flex-col justify-center text-white hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-300 transition-all">
            ...
            <h2 class="text-3xl font-extrabold"><?php echo e(number_format($stats['total_views'], 0, ',', '.')); ?></h2>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="font-bold text-slate-800 text-lg">Tren Publikasi <?php echo e(date('Y')); ?></h3>
            ...
            <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100">
                ...
                <div class="space-y-3">
                    <?php $__empty_1 = true; $__currentLoopData = $popularNews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $news): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <a href="<?php echo e(route('superadmin.news.show', $news->slug)); ?>" class="flex gap-4 items-center p-3 hover:bg-slate-50 rounded-2xl transition-all border border-transparent hover:border-slate-200 group">
                        <div class="w-16 h-16 rounded-xl bg-slate-200 flex-shrink-0 bg-cover bg-center" style="background-image: url('<?php echo e($news->thumbnail ? asset('storage/'.$news->thumbnail) : 'https://placehold.co/150x150?text=News'); ?>')"></div>
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-slate-800 text-sm truncate group-hover:text-[#5442F5] transition-colors"><?php echo e($news->title); ?></h4>
                            <div class="flex items-center gap-3 mt-1 text-xs font-medium text-slate-400">
                                <span class="text-[#5442F5]"><?php echo e($news->category->name ?? 'Uncategorized'); ?></span>
                                <span class="flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> <?php echo e($news->published_at ? $news->published_at->diffForHumans() : 'Belum rilis'); ?></span>
                            </div>
                        </div>
                        <div class="flex flex-col items-end pr-2">
                            <span class="font-extrabold text-slate-700 text-base"><?php echo e(number_format($news->views_count, 0, ',', '.')); ?></span>
            ...
            <div class="bg-gradient-to-br from-[#5442F5] to-indigo-700 rounded-[30px] p-6 shadow-xl shadow-indigo-200 text-white relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col gap-3">
                        <a href="<?php echo e(route('superadmin.news.categories.index')); ?>" class="flex items-center justify-between px-4 py-3 bg-white/10 hover:bg-white text-white hover:text-[#5442F5] rounded-xl text-sm font-bold backdrop-blur-sm transition-all border border-white/20">
                            Kelola Kategori
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        </a>
                        <a href="<?php echo e(route('superadmin.news.tags.index')); ?>" class="flex items-center justify-between px-4 py-3 bg-white/10 hover:bg-white text-white hover:text-[#5442F5] rounded-xl text-sm font-bold backdrop-blur-sm transition-all border border-white/20">
                            Kelola Tag
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        </a>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const monthlyData = <?php echo e(Illuminate\Support\Js::from($monthlyChartData)); ?>;
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/news/dashboard.blade.php ENDPATH**/ ?>