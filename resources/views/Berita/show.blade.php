...
<section style="padding-top: 140px;" class="pb-20 px-4 sm:px-6 lg:px-12 max-w-[1400px] mx-auto min-h-screen w-full">
    ...
    <div class="flex flex-col lg:flex-row gap-8 lg:gap-[3%] items-start w-full">
        ...
        <article class="layout-kiri w-full bg-white rounded-3xl p-6 sm:p-10 border border-slate-100 shadow-sm">
            ...
            <a href="{{ url('/#berita') }}" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-[#5442F5] transition-colors mb-8">
            ...
            <div class="flex flex-wrap items-center gap-4 text-sm font-semibold text-slate-500 mb-8 pb-8 border-b border-slate-100">
                <div class="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-full text-slate-700 border border-slate-200">
                    <span>✍️</span> {{ optional($news->author)->name ?? optional($news->penulis)->name ?? 'Tim Kominfo' }}
                </div>
                <div class="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-full text-slate-700 border border-slate-200">
                    <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    {{ \Carbon\Carbon::parse($news->created_at ?? $news->date)->translatedFormat('l, d F Y') }}
                </div>
            </div>
            ...
            <div class="sticky top-32">
                ...
                <div class="flex flex-col gap-4">
                    @forelse($beritaLainnya as $lainnya)
                        ...
                        <a href="{{ url('/berita/' . ($lainnya->slug ?? $lainnya->id)) }}" class="group flex items-start gap-3 bg-white p-3 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 hover:border-[#5442F5]/30">
                            
                            <!-- Thumbnail Preview -->
                            <div class="w-16 h-16 sm:w-20 sm:h-20 shrink-0 rounded-lg bg-slate-50 border border-slate-100 overflow-hidden relative">
                                @if(!empty($gambarLain))
                                    <img src="{{ $pathLain }}" alt="Thumbnail" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                @else
                            ...
                            <div class="flex flex-col pt-0.5">
                                <span class="text-[9px] font-bold text-[#5442F5] uppercase tracking-wider mb-1.5 flex items-center gap-1">
                                    ...
                                    {{ \Carbon\Carbon::parse($lainnya->created_at ?? $lainnya->date)->translatedFormat('d M Y') }}
                                </span>
