...
<?php $__env->startSection('title', 'Pendaftaran Mahasiswa'); ?>
...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10" x-data="{ modalOpen: false, currentApp: {}, actionUrl: '' }">
    ...
    <form action="<?php echo e(route('superadmin.membership.applications.index')); ?>" method="GET" class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-3">
        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama mahasiswa atau NIM..." class="flex-1 bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-2.5">
        ...
        <select name="status" class="bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-2.5 w-full sm:w-52">
            ...
            <option value="Terdaftar" <?php echo e(request('status') == 'Terdaftar' ? 'selected' : ''); ?>>Terdaftar / Menunggu</option>
            <option value="Perlu Revisi Data" <?php echo e(request('status') == 'Perlu Revisi Data' ? 'selected' : ''); ?>>Perlu Revisi Data</option>
            <option value="Ditolak" <?php echo e(request('status') == 'Ditolak' ? 'selected' : ''); ?>>Ditolak</option>
            <option value="Disetujui" <?php echo e(request('status') == 'Disetujui' ? 'selected' : ''); ?>>Disetujui (Akun Aktif)</option>
        </select>
    ...
    <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-slate-600 divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/40 transition-colors">
                        <td class="px-6 py-4">
                            <div class="font-bold text-slate-800 text-sm"><?php echo e($app->name); ?></div>
                            <div class="text-xs text-slate-400 font-medium mt-0.5">NIM: <?php echo e($app->nim); ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="font-medium text-slate-700"><?php echo e($app->email); ?></div>
                            <div class="text-xs text-slate-400 mt-0.5"><?php echo e($app->phone); ?></div>
                        </td>
                        <td class="px-6 py-4 font-medium">
                            <div><?php echo e($app->study_program); ?></div>
                            <div class="text-xs text-slate-400 mt-0.5">Angkatan <?php echo e($app->cohort); ?></div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <?php if($app->status === 'Terdaftar' || $app->status === 'Menunggu Verifikasi'): ?>
                            ...
                            <?php elseif($app->status === 'Perlu Revisi Data'): ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <button type="button" @click="modalOpen = true; currentApp = <?php echo e(json_encode($app)); ?>; actionUrl = '<?php echo e(route('superadmin.membership.applications.action', $app->id)); ?>'" class="px-4 py-1.5 bg-[#5442F5] hover:bg-[#4331e5] text-white font-bold text-xs rounded-xl shadow-xs transition-all">
                                Kelola Aksi
                            </button>
                        </td>
                    </tr>
        ...
        <div class="p-4 border-t border-slate-100 bg-slate-50/30"><?php echo e($applications->links()); ?></div>
    </div>
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/membership/applications.blade.php ENDPATH**/ ?>