...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10" x-data="{ modalOpen: false, isEdit: false, formData: {}, actionUrl: '' }">
    ...
    <div class="flex justify-between items-center">
        ...
        <button @click="modalOpen = true; isEdit = false; formData = {status: 'Persiapan'}; actionUrl = '{{ route('superadmin.masterdata.periods.store') }}'" class="px-5 py-2.5 bg-[#5442F5] hover:bg-[#4331e5] text-white rounded-xl font-bold text-sm shadow-md transition-all flex items-center gap-2">
            + Tambah Periode
        </button>
    </div>

    @if(session('success'))
        <div class="bg-emerald-50 text-emerald-600 px-4 py-3 rounded-xl text-sm font-bold">{{ session('success') }}</div>
    @endif
    ...
    <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm overflow-hidden">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm text-slate-600 divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/40 transition-colors">
                    <td class="px-6 py-4">
                        <div class="font-extrabold text-slate-800 text-base">{{ $p->name }}</div>
                        <div class="text-xs text-[#5442F5] font-bold mt-0.5">{{ $p->start_year }} - {{ $p->end_year }}</div>
                    </td>
                    <td class="px-6 py-4 font-medium text-slate-500">
                        {{ $p->start_date ? $p->start_date->format('d M Y') : 'Belum disetel' }} <br> 
                        <span class="text-xs text-slate-400">s/d</span> <br>
                        {{ $p->end_date ? $p->end_date->format('d M Y') : 'Belum disetel' }}
                    </td>
                    <td class="px-6 py-4">
                        @if($p->status === 'Aktif')
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <!-- Tombol Detail -->
                            <a href="{{ route('superadmin.masterdata.periods.show', $p->id) }}" class="p-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg transition-colors" title="Lihat Detail Visi Misi">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                            </a>
                            
                            <!-- Tombol Edit Modal -->
                            <button @click="modalOpen = true; isEdit = true; formData = {name: '{{ addslashes($p->name) }}', start_year: '{{ $p->start_year }}', end_year: '{{ $p->end_year }}', start_date: '{{ $p->start_date ? $p->start_date->format('Y-m-d') : '' }}', end_date: '{{ $p->end_date ? $p->end_date->format('Y-m-d') : '' }}', status: '{{ $p->status }}', theme: '{{ addslashes($p->theme) }}', vision: '{{ addslashes($p->vision) }}', mission: '{{ addslashes($p->mission) }}'}; actionUrl = '{{ route('superadmin.masterdata.periods.update', $p->id) }}'" class="p-2 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-lg transition-colors" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </button>

                            <!-- Tombol Aksi Status -->
                            @if($p->status !== 'Aktif')
                                <form action="{{ route('superadmin.masterdata.periods.activate', $p->id) }}" method="POST" onsubmit="return confirm('Peringatan: Mengaktifkan periode ini otomatis akan mengarsipkan periode yang sedang aktif saat ini. Lanjutkan?');">
                                    @csrf <button class="p-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-lg font-bold text-[10px] uppercase" title="Aktifkan Periode Ini">Aktifkan</button>
                                </form>
                            @else
                                <form action="{{ route('superadmin.masterdata.periods.archive', $p->id) }}" method="POST" onsubmit="return confirm('Arsipkan periode ini?');">
                                    @csrf <button class="p-2 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-lg font-bold text-[10px] uppercase" title="Arsipkan">Arsipkan</button>
                                </form>
                            @endif
                        </div>
                    </td>
        ...
        <div class="p-4 border-t border-slate-100">{{ $periods->links() }}</div>
    </div>
