...
@section('title', 'Dashboard Program Kerja')
...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100">
            ...
            <h2 class="text-3xl font-extrabold text-slate-800">{{ $totalProker }}</h2>
        </div>
        <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100">
            ...
            <h2 class="text-3xl font-extrabold text-slate-800">{{ $prokerBerjalan }}</h2>
        </div>
        <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100">
            ...
            <h2 class="text-3xl font-extrabold text-slate-800">{{ $prokerSelesai }}</h2>
        </div>
        <div class="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100">
            ...
            <h2 class="text-3xl font-extrabold text-slate-800">{{ $prokerTerlambat }} <span class="text-lg text-slate-300 font-medium">/ {{ $prokerDibatalkan }}</span></h2>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-[#5442F5] rounded-[30px] p-8 text-white shadow-xl shadow-indigo-200 lg:col-span-2 grid grid-cols-2 gap-4">
            <div>
                ...
                <h3 class="text-3xl font-extrabold tracking-tight">Rp {{ number_format($totalAnggaran, 0, ',', '.') }}</h3>
            </div>
            <div>
                <p class="text-indigo-200 text-sm font-medium mb-1">Total Realisasi</p>
                <h3 class="text-3xl font-extrabold tracking-tight">Rp {{ number_format($totalRealisasi, 0, ',', '.') }}</h3>
            </div>
            
            <div class="col-span-2 mt-4 pt-6 border-t border-white/20">
                <div class="flex justify-between text-sm mb-2 font-medium">
                    <span>Penyerapan Anggaran</span>
                    <span>{{ $persentasePenyerapan }}%</span>
                </div>
                <div class="w-full bg-black/20 rounded-full h-2.5">
                    <div class="bg-[#2CE574] h-2.5 rounded-full" style="width: {{ $persentasePenyerapan }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[30px] p-8 shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center">
            ...
            <div class="relative w-32 h-32 flex items-center justify-center">
                <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path class="text-[#5442F5]" stroke-dasharray="{{ $persentasePenyelesaian }}, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="4"></path>
                </svg>
                <div class="absolute text-2xl font-extrabold text-slate-800">{{ $persentasePenyelesaian }}%</div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const eventsData = {{ Illuminate\Support\Js::from($calendarEvents) }};
