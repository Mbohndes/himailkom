...
<?php $__env->startSection('title', 'Laporan Keuangan Kas'); ?>
...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black text-emerald-600">Rp <?php echo e(number_format($totalMasuk, 0, ',', '.')); ?></h3>
        </div>
        <div class="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Total Pengeluaran</p>
            <h3 class="text-2xl font-black text-red-500">Rp <?php echo e(number_format($totalKeluar, 0, ',', '.')); ?></h3>
        </div>
        <div class="bg-gradient-to-br from-indigo-50 to-blue-50 p-6 rounded-[24px] border border-indigo-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black text-indigo-700">Rp <?php echo e(number_format($saldoAkhir, 0, ',', '.')); ?></h3>
        </div>
    </div>

    <div class="bg-white p-6 rounded-[30px] shadow-sm border border-slate-100">
        <form action="<?php echo e(route('superadmin.reports.finance')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <?php if(request()->anyFilled(['period_id', 'month', 'year'])): ?>
                    <a href="<?php echo e(route('superadmin.reports.finance')); ?>" class="py-2.5 px-4 bg-red-50 text-red-600 rounded-xl text-sm font-bold transition-colors">Reset</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#F4F7FE]/50">
            ...
            <a href="<?php echo e(route('superadmin.reports.finance', array_merge(request()->all(), ['export' => 'print']))); ?>" target="_blank" class="px-5 py-2 bg-[#5442F5] hover:bg-[#4331e5] text-white rounded-xl text-xs font-bold shadow-md transition-all flex items-center gap-2">
                ...
                <tbody class="text-sm text-slate-600 divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/40">
                        <td class="px-6 py-4 font-mono text-xs text-slate-500">
                            <?php echo e($pay->paid_at ? $pay->paid_at->format('d/m/Y H:i') : '-'); ?>

                        </td>
                        <td class="px-6 py-4">
                            <div class="font-bold text-slate-800"><?php echo e($pay->user->name); ?></div>
                            <div class="text-xs text-slate-400 font-medium">NIM: <?php echo e($pay->user->nim ?? '-'); ?></div>
                        </td>
                        <td class="px-6 py-4 font-medium text-slate-700">
                            <?php echo e($pay->due->name); ?> 
                            <span class="text-xs font-bold text-slate-400 block mt-0.5">Masa Jabatan: <?php echo e($pay->due->period->name ?? '-'); ?></span>
                        </td>
                        <td class="px-6 py-4 font-bold text-xs text-slate-500 uppercase">
                            <?php echo e($pay->payment_method ?? 'Manual'); ?>

                        </td>
                        <td class="px-6 py-4 text-right font-black text-emerald-600 text-base">
                            + Rp <?php echo e(number_format($pay->amount_paid, 0, ',', '.')); ?>
...
<?php echo $__env->make('layouts.superadmin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\sim-hima\resources\views/superadmin/reports/finance.blade.php ENDPATH**/ ?>