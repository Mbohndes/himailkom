...
@section('title', isset($proker) ? 'Edit Program Kerja' : 'Tambah Program Kerja')
...
<div class="max-w-5xl mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-[28px] font-extrabold text-slate-800 tracking-tight">
                {{ isset($proker) ? 'Edit Program Kerja' : 'Buat Program Kerja' }}
            </h1>
    ...
    <form action="{{ isset($proker) ? route('superadmin.prokers.update', $proker->id) : route('superadmin.prokers.store') }}" method="POST" class="space-y-6">
        @csrf
        @if(isset($proker)) @method('PUT') @endif
        ...
        <div class="bg-white rounded-[30px] shadow-sm border border-slate-100 p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="program_code" value="{{ old('program_code', $proker->program_code ?? '') }}" required placeholder="Contoh: PK-KOM-001" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Nama Program <span class="text-red-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $proker->name ?? '') }}" required placeholder="Contoh: Seminar Nasional AI" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div>
                    ...
                    <select name="division_id" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                        ...
                        @foreach($divisions as $div)
                            <option value="{{ $div->id }}" {{ old('division_id', $proker->division_id ?? '') == $div->id ? 'selected' : '' }}>{{ $div->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="period_id" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                        @foreach($periods as $per)
                            <option value="{{ $per->id }}" {{ old('period_id', $proker->period_id ?? '') == $per->id ? 'selected' : '' }}>{{ $per->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="pic_id" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('pic_id', $proker->pic_id ?? '') == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Jenis Program <span class="text-red-500">*</span></label>
                    <input type="text" name="program_type" value="{{ old('program_type', $proker->program_type ?? '') }}" required placeholder="Contoh: Seminar, Lomba, Pelatihan" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Tanggal Mulai <span class="text-red-500">*</span></label>
                    <input type="date" name="start_date" value="{{ old('start_date', $proker->start_date ?? '') }}" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Deadline / Selesai <span class="text-red-500">*</span></label>
                    <input type="date" name="end_date" value="{{ old('end_date', $proker->end_date ?? '') }}" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Lokasi Pelaksanaan</label>
                    <input type="text" name="location" value="{{ old('location', $proker->location ?? '') }}" placeholder="Contoh: Auditorium Universitas / Zoom Meeting" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[30px] shadow-sm border border-slate-100 p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="estimated_participants" value="{{ old('estimated_participants', $proker->estimated_participants ?? 0) }}" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Rencana Anggaran (Rp)</label>
                    <input type="number" name="budget_planned" value="{{ old('budget_planned', $proker->budget_planned ?? 0) }}" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                </div>
                <div>
                    ...
                    <select name="priority" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                        <option value="Rendah" {{ old('priority', $proker->priority ?? '') == 'Rendah' ? 'selected' : '' }}>Rendah</option>
                        <option value="Sedang" {{ old('priority', $proker->priority ?? 'Sedang') == 'Sedang' ? 'selected' : '' }}>Sedang</option>
                        <option value="Tinggi" {{ old('priority', $proker->priority ?? '') == 'Tinggi' ? 'selected' : '' }}>Tinggi</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="status" required class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">
                        <option value="Draft" {{ old('status', $proker->status ?? 'Draft') == 'Draft' ? 'selected' : '' }}>Draft (Perencanaan)</option>
                        <option value="Berjalan" {{ old('status', $proker->status ?? '') == 'Berjalan' ? 'selected' : '' }}>Sedang Berjalan</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="objective" rows="3" class="w-full bg-[#F4F7FE] border-none text-sm font-medium text-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5442F5]">{{ old('objective', $proker->objective ?? '') }}</textarea>
                </div>
            </div>
        </div>

        <div class="flex items-center justify-end gap-4 mt-6">
            <a href="{{ route('superadmin.prokers.index') }}" class="px-8 py-3.5 bg-white text-slate-600 rounded-xl font-bold text-sm shadow-sm hover:bg-slate-50 transition-colors">
                Batal
            </a>
            <button type="submit" class="px-8 py-3.5 bg-[#5442F5] hover:bg-[#4331e5] text-white rounded-xl font-bold text-sm shadow-xl shadow-indigo-200 transition-colors">
                {{ isset($proker) ? 'Simpan Perubahan' : 'Buat Program Kerja' }}
            </button>
        </div>
    </form>
</div>
