...
<div class="max-w-[1400px] mx-auto w-full flex flex-col gap-6 pb-10">
    ...
    <div class="bg-white p-6 rounded-[30px] shadow-sm border border-slate-100">
        <form action="{{ route('superadmin.reports.programs') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
            ...
            <div>
                ...
                <select name="status" class="w-full bg-[#F4F7FE] border-none text-sm font-bold text-slate-700 rounded-xl px-4 py-2.5">
                    ...
                    <option value="Selesai" {{ request('status') == 'Selesai' ? 'selected' : '' }}>Program Selesai</option>
                    <option value="Berjalan" {{ request('status') == 'Berjalan' ? 'selected' : '' }}>Program Berjalan</option>
                    <option value="Terlambat" {{ request('status') == 'Terlambat' ? 'selected' : '' }}>Program Terlambat</option>
                </select>
            </div>
            ...
            <div class="flex gap-2">
                ...
                @if(request()->anyFilled(['period_id', 'division_id', 'status']))
                    <a href="{{ route('superadmin.reports.programs') }}" class="py-2.5 px-4 bg-red-50 text-red-600 rounded-xl text-sm font-bold transition-colors">Reset</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-[30px] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#F4F7FE]/50">
            <div>
                ...
                <p class="text-xs font-medium text-slate-500 mt-0.5">Total Ditemukan: {{ $programs->count() }} program.</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('superadmin.reports.programs', array_merge(request()->all(), ['export' => 'print'])) }}" target="_blank" class="px-5 py-2 bg-[#5442F5] hover:bg-[#4331e5] text-white rounded-xl text-xs font-bold shadow-md transition-all flex items-center gap-2">
                ...
                <tbody class="text-sm text-slate-600 divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/40">
                        <td class="px-6 py-4">
                            ...
                            <div class="font-bold text-slate-800 text-sm">{{ $prog->name }}</div>
                            <div class="text-xs text-slate-400 mt-0.5">Periode: {{ $prog->period->name ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 font-bold text-slate-700">
                            {{ $prog->division->name ?? 'Umum / Pengurus Inti' }}
                        </td>
                        <td class="px-6 py-4 font-medium text-slate-500">
                            <!-- Sesuaikan 'target_date' dengan nama kolom tanggal Anda -->
                            {{ $prog->target_date ? \Carbon\Carbon::parse($prog->target_date)->format('d M Y') : '-' }}
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-24 bg-slate-100 rounded-full h-2 overflow-hidden">
                                    ...
                                    <div class="bg-indigo-500 h-full rounded-full" style="width: {{ $prog->progress_percent ?? 0 }}%"></div>
                                </div>
                                <span class="text-xs font-extrabold text-slate-700">{{ $prog->progress_percent ?? 0 }}%</span>
                            </div>
                        </td>
